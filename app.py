...
def login(username: str = Form(...), password: str = Form(...)):
    ...
    for _, row in df.iterrows():
        ...
        # Card do Funil Kanban (Mantido igual)
        status_atual = KANBAN_STATE.get(cid, "afazer")
        card_html = f"""
        <div data-id="{cid}" class="bg-slate-950 border border-slate-800 p-3.5 rounded-xl space-y-2 cursor-grab active:cursor-grabbing shadow-sm hover:border-slate-700 transition duration-150">
            <div class="flex justify-between items-center"><span class="text-[11px] font-bold text-white">#{cid}</span><span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-slate-900 text-slate-400 border border-slate-800">{tickets} Tickets</span></div>
            <p class="text-[11px] font-medium text-slate-400 truncate">{segmento}</p>
