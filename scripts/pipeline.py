...
# ==========================================
    # ETAPA 3 - MODELAGEM DE GAPS CORRIGIDA (ANTIGAFE)
    # ==========================================
    def model_opportunities(self):
        print("--- ETAPA 3: Modelagem de Oportunidades (Filtro Rigoroso) ---")
        ...
        # 2. Criar dicionário de serviços ativos por cliente
        ...
        # 3. Varrer dores e checar o GAP exato no contrato
        for _, row in recorrencia.iterrows():
            ...
            # --- REGRAS ULTRA RESTRITIVAS (FILTRO DE GAPS E VOLUMETRIA) ---
            
            # BACKUP: Elevada a régua para só oferecer se tiver 4 ou mais problemas
            if categoria == "BACKUP" and "BACKUP" not in texto_contratos and qtd >= 4:
                gap_encontrado = True
                servico_sugerido = "Backup Gerenciado"
                
            # SEGURANÇA (CORREÇÃO CIRÚRGICA): 
            # Só sugere o Pacote de Firewall se ele REALMENTE não tiver a palavra FIREWALL ou FORTINET no contrato
            elif categoria == "SEGURANÇA":
                if not any(w in texto_contratos for w in ["FIREWALL", "FORTINET"]) and qtd >= 3:
                    gap_encontrado = True
                    servico_sugerido = "Pacote de Cibersegurança / Firewall"
                
            # INTERNET/REDE: Elevada a régua para 6 chamados (gargalo crítico)
            elif categoria == "INTERNET/REDE" and not any(w in texto_contratos for w in ["DEDICADO", "LINK", "REDE"]) and qtd >= 6:
                gap_encontrado = True
                servico_sugerido = "Acesso Dedicado Link Corporativo"
                
            # HOSPEDAGEM/CLOUD: Elevada a régua para 5 chamados
            elif categoria == "HOSPEDAGEM/CLOUD" and not any(w in texto_contratos for w in ["CLOUD", "HOSPEDAGEM", "VIRTUAL"]) and qtd >= 5:
                gap_encontrado = True
                servico_sugerido = "Migração para Nuvem / Cloud Dedicado"

            # Se passou pelos novos filtros, valida a oportunidade
            if gap_encontrado:
